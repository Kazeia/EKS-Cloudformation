...
function aws_delete_stack() {
  ...
  local STACK_DELETE=($(eval "aws2 " $PROFILE_PARAM " " $REGION_PARAM " cloudformation delete-stack --stack-name " $STACK_NAME " " $PARAMS))
...
function aws_wait_delete_stack() {
  ...
  STACK_STATUS=$(eval "aws2 " $PROFILE_PARAM " " $REGION_PARAM " cloudformation wait stack-delete-complete --stack-name " $STACK_NAME)
